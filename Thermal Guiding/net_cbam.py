...
class LayerNorm(nn.Module):
    def __init__(self, dim, LayerNorm_type):
        ...
        if LayerNorm_type == 'BiasFree': self.body = BiasFree_LayerNorm(dim)
        else: self.body = WithBias_LayerNorm(dim)
    def forward(self, x):
        h, w = x.shape[-2:]
...
# =============================================================================
# 【核心创新工作点 2】跨模态热引导注意力融合模块 (Thermal-Guided Fusion)
# 彻底淘汰简单相加，利用红外空间特征引导可见光融合
# =============================================================================
class ThermalGuidedBaseFusion(nn.Module):
    def __init__(self, dim=64, num_heads=8, ffn_expansion_factor=1.):
        super(ThermalGuidedBaseFusion, self).__init__()
        # 使用 Spatial Attention 提取红外热掩码
        self.spatial_att = SpatialAttention(kernel_size=7)
        # 降维用的 1x1 卷积
        self.agg_conv = nn.Conv2d(dim*2, dim, kernel_size=1)
        # 维持原有的深层特征提取器
        self.processor = BaseFeatureExtraction(dim, num_heads, ffn_expansion_factor)
        
    def forward(self, f_vi, f_ir):
        # 1. 提取红外热辐射空间掩码 (Thermal Spatial Mask)
        thermal_mask = self.spatial_att(f_ir)
        
        # 2. 跨模态引导：用红外热辐射强度去强化可见光的对应细节
        f_vi_guided = f_vi * thermal_mask + f_vi
        
        # 3. 特征深度聚合 (拼接后降维，取代原本的相加)
        f_fused = torch.cat([f_vi_guided, f_ir], dim=1)
        f_fused = self.agg_conv(f_fused)
        
        # 4. 进入原有的深层提取模块
        return self.processor(f_fused)

class ThermalGuidedDetailFusion(nn.Module):
    def __init__(self, dim=64, num_layers=1):
        super(ThermalGuidedDetailFusion, self).__init__()
        self.spatial_att = SpatialAttention(kernel_size=7)
        self.agg_conv = nn.Conv2d(dim*2, dim, kernel_size=1)
        self.processor = DetailFeatureExtraction(num_layers=num_layers)
