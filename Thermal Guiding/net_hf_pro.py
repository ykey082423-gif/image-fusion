...
class IAM(nn.Module):
    ...
    def forward(self, x):
        ...
        w = self.fc(out)
        w = torch.sigmoid(w) # 强制挤压到 0~1 之间
...
class Restormer_Decoder_HF_Pro(nn.Module):
    ...
    def forward(self, inp_img, ir_img, base_feature, detail_feature):
        ...
        # ⚠️ 绝对关键：兼容 Phase I (重建阶段) 的输入 ⚠️
        if ir_img is None:
            return self.sigmoid(out + inp_img), out_enc_level0
        if inp_img is None:
            return self.sigmoid(out + ir_img), out_enc_level0
            
        # 🟢 Phase II: 真正开始光照感知与融合 🟢
        w = self.iam(inp_img) 
        ir_low = self.smooth(ir_img)
        ir_high = ir_img - ir_low
        
        # 动态自适应公式
        final_out = self.sigmoid(out + w * inp_img + (2.0 - w) * ir_high)
